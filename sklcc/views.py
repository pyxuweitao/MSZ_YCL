...
def edit(request, Id):
    ...
    jsonContent = json.loads( rawJson, encoding='utf-8' )
...
def users_info_operations(request):
    ...
    if request.method == 'GET':
        cursor.execute("SELECT * FROM RMI_ACCOUNT_USER")
        raws = cursor.fetchall()
        col_names = [desc[0] for desc in cursor.description]
        if not raws:
            return HttpResponseNotFound()
        user_list = [dict(zip(col_names, raw)) for raw in raws]
        return HttpResponse(json.dumps(user_list, encoding='GB2312'), content_type="application/json")
    if request.method == 'POST':
        data = json.loads(request.body)
        # mark
        Id = set_randhex('Id', 'RMI_ACCOUNT_USER')
        cursor.execute(
            "insert into RMI_ACCOUNT_USER (Id,[Name],Password,Avatar,Department,Job,Permission,CreateTime,LastModifiedTime) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
            Id, data['Name'], data['Password'], data['Avatar'], data['Department'], data['Job'], data['Permission'],
...
def get_form(request):
    if request.method == 'GET':
        ...
        cursor.execute(
            ...
            randhex, data['ArrivalSerialNumber'], data['ProcessTypeId'], data['ProcessNodeTypeId'], data['Status'],
            request.session['UserId'], int(time.time() * 1000), int(time.time() * 1000), request.session['UserId'],
            data['Rev']))
        ...
        if 'addToArrival' in data and data['addToArrival'] == 'true':
            ...
            for ele in parameter:
                ...
                cursor.execute(
                    ...
                    randhex, process_type_id[0], id, int(time.time() * 1000), json.dumps(ele, encoding='GB2312')))
        ...
        if 'addToArrivalAndAddToProcess' in data and data['addToArrivalAndAddToProcess'] == 'true':
            ...
            for ele in parameter:
                ...
                cursor.execute(
                    ...
                    randhex, process_type_id[0], int(time.time() * 1000), rev, json.dumps(ele, encoding='GB2312')))
                cursor.execute(
                    "insert into RMI_ARRIVAL(ArrivalSerialNumber,ProcessTypeId,SourceFormId,OutFlag,InTime,Parameter) values('%s','%s','%s','false','%s','%s')" % (
                    randhex, process_type_id[0], id, int(time.time() * 1000), json.dumps(ele, encoding='GB2312')))
    ...
    if request.method == 'POST' and 'ProcessTypeId' in request.POST and 'Rev' in request.POST:
        ...
        if 'StartTime' in request.POST and request.POST['StartTime'] and 'EndTime' in request.POST and request.POST[
            ...
            cursor.execute(
                ...
                request.POST['ProcessTypeId'], request.POST['Rev'], request.POST['StartTime'], request.POST['EndTime']))
...
def add_arrival_process(request, ASN):
    ...
    if request.method == 'POST':
        ...
        if not len(raw):
            return HttpResponseBadRequest()
        cursor.execute(
            "insert into RMI_PROCESS(ArrivalSerialNumber,TypeId,IsComplete) values('%s','%s','false')" % (ASN, raw[0]))
        cursor.execute("update RMI_ARRIVAL set OutFlag = 'true',OutTime = '%s' where ArrivalSerialNumber = '%s'" % (
